...
**DIGITAL MARKET** ابزار مصاحبه هدایت‌شده و تدوین پیشنهادهای استراتژی و هویت برند است. اجرای محلی از پاسخ‌های ثبت‌شده، قواعد وابستگی سؤال‌ها، مسیریاب ۱۵ محوره و ۴۹ گره ثبت‌شده در ویکی استفاده می‌کند. بازنویسی سؤال با AI اختیاری است؛ مرجع داخلی ویکی به معنای دسترسی زنده به داده‌های بازار ایران نیست.

فرم مالی فاز اول، حاشیه مشارکت، فروش سربه‌سر و مازاد یا کسری ماهانه را با واحد و دوره مشخص محاسبه می‌کند. تغییر هزینه، ظرفیت یا مسئله مشتری، سؤال‌ها و اقدامات مرتبط را تغییر می‌دهد. جزئیات و محدودیت‌ها در گزارش فارسی بالا آمده است. برای آزمون واقعی مرورگر، در پوشه `platform` پس از `npm ci`، دستورهای `npx playwright install --with-deps chromium` و `npm run test:browser` را اجرا کنید.
