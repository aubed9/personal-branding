...
export default function ContextSummaryLedger({
  ...
  const activeGuild = businessContext?.resolvedType || (businessContext?.taxonomyId ? {
    id: businessContext.taxonomyId, titleFa: businessContext.taxonomyTitleFa,
    industryId: businessContext.industryId,
  } : null);
  const axes = businessContext?.axes || {};
  const activeContradictions = contradictions.filter(c => !c.resolutionStatus || c.resolutionStatus === 'UNRESOLVED');
  const pendingItems = [...unknowns.filter(u => u.status !== 'RESOLVED'), ...assumptions.filter(a => a.kind === 'ASSUMPTION')];
  const readable = item => typeof item === 'string' ? item : item.statement || item.text || item.reason || item.descriptionFa || item.description || item.decision || 'توضیحی ثبت نشده است.';
      ...
      <div className="p-4 border-b border-white/10 shrink-0 bg-[#0A0A0A]">
        ...
        {businessContext && (
          <div className="flex flex-wrap gap-1 mt-2.5">
            ...
            {(axes.channelModel || axes.channel) && (
              <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-white/5 border border-white/10 text-zinc-300">
                {axes.channelModel || axes.channel}
              </span>
            )}
            {(axes.maturity || axes.businessStage) && (
              <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-white/5 border border-white/10 text-zinc-300">
                {axes.maturity || axes.businessStage}
              </span>
            )}
            {(axes.geography || axes.geographicScale) && (
              <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-white/5 border border-white/10 text-zinc-300">
                {axes.geography || axes.geographicScale}
              </span>
            )}
      ...
      {activeContradictions.length > 0 && (
        <div className="p-3 bg-[#160b0b] border-b border-red-950 text-white shrink-0">
          ...
          <p className="text-[11px] text-zinc-300 leading-relaxed">
            {activeContradictions[0]?.resolutionQuestion || activeContradictions[0]?.descriptionFa || "تناقض در اظهارات نیازمند بازبینی و یکپارچه‌سازی است."}
          </p>
        </div>
      )}
        ...
        <div>
          ...
          >
            <div className="flex items-center gap-1.5">
              ...
              <span>اطلاعات اعلام‌شده شما ({facts.length})</span>
            </div>
          ...
          {isFactsOpen && (
            <div className="mt-1 space-y-1.5 pl-1">
              {facts.length === 0 ? (
                <p className="text-[11px] text-zinc-500 italic py-1">هنوز اطلاعاتی ثبت نشده است.</p>
              ) : (
        ...
        <div className="pt-3">
          ...
          >
            <div className="flex items-center gap-1.5">
              ...
              <span>مجهولات و فرضیات ({pendingItems.length})</span>
            </div>
          ...
          {isUnknownsOpen && (
            <div className="mt-1 space-y-1.5 pl-1">
              {pendingItems.length === 0 ? (
                ...
                pendingItems.slice(-5).map((unk, idx) => {
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[9px] font-mono px-1.5 py-0.2 rounded border border-white/20">
                          {isBlocking ? "مجهول بحرانی" : unk.kind === 'ASSUMPTION' ? "فرضیه نیازمند آزمون" : "داده نامعلوم"}
                        </span>
